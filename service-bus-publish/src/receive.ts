...
import { delay, ProcessErrorArgs, ServiceBusClient, ServiceBusReceivedMessage, ServiceBusSessionReceiverOptions } from "@azure/service-bus";
...
async function main(): Promise<void> {
  ...
  const maxConcurrentCalls = process.argv.length > 2 ? parseInt(process.argv[3]) : 10;
  const messages = process.argv.length > 3 ? parseInt(process.argv[4]) : 100;
  const isReceiveAndDelete = process.argv.length > 4 ? !(process.argv[5] === "false") : true;
